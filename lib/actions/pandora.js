...
function pandora(player, values) {
  ...
  function playPandora(player, name) {
    ...
    return userLogin()
      .then(() =>
        pandoraAPI('user.getStationList', { includeStationArtUrl: true }),
      )
      .then(stationList => {
        return pandoraAPI('music.search', { searchText: name })
          ...
          .then(result => {
            ...
            const fuzzy = new Fuse(stationList.stations, {
              keys: ['stationName'],
            });
            ...
            if (results.length > 0) {
              ...
              if (station.type == undefined) {
                ...
                metadata = getPandoraMetadata(
                  station.stationId,
                  station.stationName,
                  player.system.getServiceType('Pandora'),
                );
                return Promise.resolve();
              }
              return pandoraAPI('station.createStation', {
                musicToken: station.stationId,
                musicType: station.type,
              }).then(stationInfo => {
  ...
  if (settings && settings.pandora) {
    var pAPI = new Anesidora(
      settings.pandora.username,
      settings.pandora.password,
    );
    ...
    if (cmd == 'thumbsup' || cmd == 'thumbsdown') {
      const sid = player.system.getServiceId('Pandora');
      const { uri } = player.state.currentTrack;
      ...
      if (
        ...
        parameters.sid == sid &&
        player.state.currentTrack.trackUri
      ) {
        const { trackUri } = player.state.currentTrack;
        const trackToken = trackUri.substring(
          trackUri.search('x-sonos-http:') + 13,
          trackUri.search('%3a%3aST%3a'),
        );
        const stationToken = trackUri.substring(
          trackUri.search('%3a%3aST%3a') + 11,
          trackUri.search('%3a%3aRINCON'),
        );
        const up = cmd == 'thumbsup';

        return userLogin()
          .then(() =>
            pandoraAPI('station.addFeedback', {
              stationToken,
              trackToken,
              isPositive: up,
            }),
      ...
      return Promise.reject(
        'The music that is playing is not a Pandora station',
      );
    }
  } else {
