...
function spotify(player, values) {
  ...
  const metadata = getSpotifyMetadata(
    encodedSpotifyUri,
    player.system.getServiceType('Spotify'),
  );
  ...
  if (action == 'queue') {
    ...
    if (
      ...
      promise = promise.then(() =>
        player.coordinator.setAVTransport(
    ...
    return promise.then(() => {
      return player.coordinator
        .addURIToQueue(uri, metadata, true, nextTrackNo)
        .then(addToQueueStatus =>
          player.coordinator.trackSeek(
            addToQueueStatus.firsttracknumberenqueued,
          ),
        )
