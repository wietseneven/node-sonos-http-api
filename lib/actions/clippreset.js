...
function playClipOnPreset(player, values) {
  ...
  if (!preset) {
    return Promise.reject(
      new Error(`No preset named ${presetName} could be found`),
    );
  }

  return fileDuration(path.join(LOCAL_PATH_LOCATION, clipFileName)).then(
    duration => {
      return presetAnnouncement(
        player.system,
        `http://${player.system.localEndpoint}:${port}/clips/${clipFileName}`,
        preset,
        duration,
      );
    },
