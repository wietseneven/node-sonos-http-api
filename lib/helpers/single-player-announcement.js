...
function singlePlayerAnnouncement(player, uri, volume, duration) {
  ...
  const ttsPreset = {
    players: [{ roomName: player.roomName, volume }],
    playMode: {
      repeat: false,
    },
    uri,
  ...
  const prepareBackupPreset = () => {
    if (backupPresets[player.roomName].length > 1) {
      ...
      logger.debug(
        'more than 1 backup presets during prepare',
        backupPresets[player.roomName],
      );
    ...
    if (relevantBackupPreset.group) {
      const zone = system.zones.find(
        zone => zone.id === relevantBackupPreset.group,
      );
      if (zone) {
    ...
    return system.applyPreset(relevantBackupPreset).then(() => {
      backupPresets[player.roomName].shift();
      logger.debug(
        'after backup preset applied',
        backupPresets[player.roomName],
      );
    });
  };
  ...
  return system
    ...
    .then(() => {
      ...
      logger.debug(
        `${elapsed} elapsed with ${restoreTimeout - elapsed} to spare`,
      );
    ...
    .catch(err => {
      logger.error(err);
      return prepareBackupPreset().then(() => {
        throw err;
      });
    });
}
