...
function readPresetsFromDir(presets, presetPath) {
  ...
  files
    .map(name => {
      const fullPath = path.join(presetPath, name);
      return {
        name,
        fullPath,
    ...
    .forEach(file => {
      ...
      if (Object.keys(preset).length === 0) {
        logger.warn(
          `could not parse preset file ${file.name}, please make sure syntax conforms with JSON5.`,
        );
        return;
      }
...
function readPresetsFromFile(presets, filename) {
  try {
    ...
    logger.warn(
      'You are using a presets.json file! ' +
...
try {
  ...
  logger.warn(
    `Could not start watching dir ${PRESETS_PATH}, will not auto reload any presets. Make sure the dir exists`,
  );
  logger.warn(e.message);
}
