const appleDef = {
  ...
  search: {
    ...
    song:
      'https://itunes.apple.com/search?media=music&limit=50&entity=song&term=',
    station:
      'https://itunes.apple.com/search?media=music&limit=50&entity=musicArtist&term=',
  },
  metastart: {
...
function loadTracks(type, tracksJson) {
  ...
  if (tracksJson.resultCount > 0) {
    ...
    tracks.queueTracks = tracksJson.results.reduce(function(
      tracksArray,
      track,
    ) {
      if (track.isStreamable) {
        ...
        if (!skip) {
          const metadataID =
            appleDef.metastart.song + encodeURIComponent(track.trackId);
          ...
          const uri = getURI('song', encodeURIComponent(track.trackId));
