...
const getHeaders = () => {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      'You are missing spotify clientId and secret in settings.json! Please read the README for instructions on how to generate and add them',
    );
  }
  ...
  return new Promise((resolve, reject) => {
    request(options)
      ...
      .catch(err => {
        reject(
          new Error(
            `Unable to authenticate Spotify with client id: ${clientId}`,
          ),
        );
      });
  });
...
function getURIandMetadata(type, resList) {
  ...
  UaM.uri = getURI(
    type,
    encodeURIComponent(type == 'station' ? items[0].id : items[0].uri),
  );
...
function loadTracks(type, tracksJson) {
  ...
  if (tracksJson.tracks.items.length > 0) {
    ...
    tracks.queueTracks = tracksJson.tracks.items.reduce(function(
      tracksArray,
      track,
    ) {
      if (
        ...
        if (!skip) {
          const metadataID =
            spotifyDef.metastart.song + encodeURIComponent(track.id);
