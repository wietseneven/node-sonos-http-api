...
const randomQueueLimit =
  settings.library && settings.library.randomQueueLimit !== undefined
    ? settings.library.randomQueueLimit
    : 50;
...
function loadLibrary(player) {
  ...
  const getChunk = chunk => {
    chunk.items.reduce(function(tracksArray, item) {
      if (
        ...
        const metadataID =
          libraryDef.metastart.song +
          item.uri.substring(item.uri.indexOf(':') + 1);
    ...
    logger.info(
      `Tracks returned: ${result.numberReturned}, Total matches: ${result.totalMatches}`,
    );
    ...
    return player
      .browse('A:TRACKS', chunk.startIndex + chunk.numberReturned, 0)
      .then(getChunk);
  };
  ...
  return Promise.resolve(result)
    ...
    .catch(err => {
      logger.error(
        'Error when recursively trying to load library using browse()',
        err,
      );
    });
}
...
function loadLibrarySearch(player, load) {
  if (load || musicLibrary == null) {
    return loadLibrary(player)
      ...
      .then(() =>
        loadFuse(musicLibrary.tracks.items, [
          'artistTrackSearch',
          'artistName',
          'trackName',
        ]),
  ...
  return loadFuse(musicLibrary.tracks.items, [
    'artistTrackSearch',
    'artistName',
    'trackName',
  ])
...
function searchLibrary(type, term) {
  ...
  return type == 'album'
    ? fuzzyAlbums.search(term)
    : fuzzyTracks
        .search(term)
        .shuffle()
        .slice(0, randomQueueLimit);
}
...
function handleLibrary(err, data) {
  if (!err) {
    ...
    if (
      musicLibrary.version == undefined ||
      musicLibrary.version < currentLibVersion
    ) {
